...
class CommuteService{
    ...
    myRideRequestsAPI(dataToGetRequests){
        let requests={
                    requests:rideRequestData.requests.filter((request,index)=>(index>=dataToGetRequests.offset && index<dataToGetRequests.offset+dataToGetRequests.limit)),
    ...
    myAssetRequestsAPI(dataToGetRequests){
        let requests={
                    requests:assetRequestData.requests.filter((request,index)=>(index>=dataToGetRequests.offset && index<dataToGetRequests.offset+dataToGetRequests.limit)),
    ...
    matchingAllRequestsAPI(dataToGetMatchingRequests){
        return new Promise((resolve)=>{
           setTimeout(()=>{
                resolve(allRequestsData.assetRequests.concat(allRequestsData.rideRequests));
            },1000);
        });
    }
