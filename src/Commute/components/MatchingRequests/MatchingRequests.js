...
import {Header,Title,DropDown,MatchingRequestsDisplay,CardsDisplay} from './styledComponents.js';
import LoadingWrapperWithFailure from '../../../components/common/LoadingWrapperWithFailure';

import {MatchingRideRequestCard,MatchingAssetRequestCard} from '../../stores/Models/MatchingRequestCard';
...
class MatchingRequests extends React.Component{
        ...
        return (
            ...
            {this.getMatchingRequests().map(request=>{
                if(request.typeOfRequest==='RIDE'){
                    return (
                    <RideRequest key={Math.random()+request.id} requestData={request} />
                    );
                }
                else{
                    return (
                    <AssetRequest key={Math.random()+request.id} requestData={request} />
                    );
                }
            })}
            </CardsDisplay>
    ...
    renderSuccessUI(){
        const {onChangeFilter,renderCardRequestCards}=this;
         const {getMatchingRequestAPIStatus,getMatchingRequestAPIError,doNetworkCalls}=this.props;
        const filterOptions={
            listTitle:'',
        ...
        return (
            <MatchingRequestsDisplay>
                ...
                <LoadingWrapperWithFailure key={this.navigateTo} apiStatus={getMatchingRequestAPIStatus} apiError={getMatchingRequestAPIError} 
                            onRetryClick={doNetworkCalls} renderSuccessUI={renderCardRequestCards} 
                        />
            </MatchingRequestsDisplay>
            ); 
    }
